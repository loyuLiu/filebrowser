...
// RenameStore is the interface for accessing rename metadata.
type RenameStore interface {
	GetOriginalNames(dirPath string) map[string]string
}
...
func NewFileInfo(opts *FileOptions) (*FileInfo, error) {
	...
	if opts.Expand {
		if file.IsDir {
			if err := file.readListing(opts.Checker, opts.ReadHeader, opts.CalcImgRes, opts.RenameStore); err != nil {
				return nil, err
			}
			return file, nil
		}
...
func (i *FileInfo) readListing(checker rules.Checker, readHeader bool, calcImgRes bool, renameStore RenameStore) error {
	...
	// Load rename metadata for this directory
	var renameMap map[string]string
	if renameStore != nil {
		renameMap = renameStore.GetOriginalNames(i.Path)
	} else {
